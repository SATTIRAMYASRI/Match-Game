...
class App extends Component {
  ...
  updateMatchImg = () => {
    ...
    this.setState({matchImg: imagesList[randomNumber]})
  }
  ...
  getThumbNailImg = id => {
    const {matchImg, score} = this.state
    if (matchImg.id === id) {
      this.setState({
        score: score + 1,
      })
  ...
  render() {
    const {matchImg, activeTabId, score, gameRunning, time} = this.state
    const filteredImgList = this.getFilteredList()
    return (
      <div className="bg-container">
        ...
        {gameRunning ? (
          <Game
            tabsList={tabsList}
            filteredImgList={filteredImgList}
            matchImg={matchImg}
            activeTabId={activeTabId}
            updateActiveTabId={this.updateActiveTabId}
            getThumbNailImg={this.getThumbNailImg}
          />
        ) : (
